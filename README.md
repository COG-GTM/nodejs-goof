...
On first startup Goof provisions an admin user. No credential is committed to this repository:
set `ADMIN_USERNAME` (default `admin@example.com`) and `ADMIN_PASSWORD` before starting, or let the
app generate a random one-time password which it prints to the log once:

```
Provisioned admin user admin@example.com with the one-time password: <random>
```

Only a scrypt hash of the password is stored, so a lost password means dropping the `users`
collection and letting it be re-provisioned.
...
```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary "{\"username\": \"$ADMIN_USERNAME\", \"password\": \"$ADMIN_PASSWORD\"}" 'http://localhost:3001/login'
```

```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"email": "admin@example.com", "firstname": "admin", "lastname": "admin", "country": "IL", "phone": "+972551234123",  "layout": "./../package.json"}' 'http://localhost:3001/account_details'
```
...
It works by exposing `loginHandler` as a controller in `routes/index.js` and uses a MongoDB database to look up the user's details (email as a username) and then verifies the submitted password against the stored scrypt hash.

We can send a request with an incorrect password to see that we get a failed attempt
```sh
echo '{"username":"admin@example.com", "password":"WrongPassword"}' | http --json $GOOF_HOST/login -v
```

And another request, using the provisioned admin credentials, signs in as expected:
```sh
echo "{\"username\":\"$ADMIN_USERNAME\", \"password\":\"$ADMIN_PASSWORD\"}" | http --json $GOOF_HOST/login -v
```
...
```sh
echo '{"username": "admin@example.com", "password": {"$gt": ""}}' | http --json $GOOF_HOST/login -v
```

We know the username, and we pass on what seems to be an object of some sort.
Note that `loginHandler` now rejects non-string credentials and compares the submitted password against a stored hash, so this payload no longer authenticates. Historically that object structure was passed as-is to the `password` property, and it has a specific meaning to MongoDB - it uses the `$gt` operation which stands for `greater than`. So, we in essence tell MongoDB to match that username with any record that has a password that is greater than `empty string` which is bound to hit a record. This introduces the NoSQL Injection vector.
