...
tap.test('AdmZip.extractAllTo (the replaced sink) does escape the destination', function (t) {
  var tmp     = tempDest();
  var escaped = tmp.escaped;

  new AdmZip(maliciousZip).extractAllTo(tmp.dest, true);

  t.ok(fs.existsSync(escaped), 'documents the vulnerable behaviour the fix replaces');
