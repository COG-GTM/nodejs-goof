...
exports.save_account_details = function(req, res, next) {
  ...
  if (validator.isEmail(profile.email, { allow_display_name: true })
    ...
    // build the view locals from an allowlist of expected fields only.
    // Passing the raw request body lets an attacker supply a `layout`
    // property, which the hbs engine resolves to a filesystem path,
    // enabling path traversal (SonarQube jssecurity:S2083).
    const safeProfile = {
